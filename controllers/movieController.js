...
// PATCH /api/movies/:id/favorite - Alterna el estado favorito de una película
...
// PATCH /api/movies/:id/rating - Actualiza la valoración de una película
exports.updateRating = async (req, res) => {
  const { id } = req.params;
  const { rating } = req.body;

  if (!Number.isInteger(rating) || rating < 0 || rating > 5) {
    return res.status(400).json({ error: 'Rating inválido. Debe ser un número entre 0 y 5' });
  }
